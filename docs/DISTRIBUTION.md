...
安装包会创建 `propel`、`propel-web` 和 `propel-desktop` 三个入口。免安装包完整解压后也包含这三个入口。工作流会先运行 Maven 测试，并实际启动 `propel.exe` 检查 `/api/health`，确认默认入口确实是网页版后才上传产物。推送形如 `v1.1.0` 的 tag 也会自动触发构建。
...
默认会执行测试并生成 `release\windows\propel-1.1.0.exe`。仅在临时诊断构建时跳过测试：
...
网页中的 Currency conversion 支持多个 source currencies 映射到一个 target currency。每个汇率表示
“1 单位 source currency = 多少单位 target currency”。后端会校验 FilteredFrames 中的每个币种均有
对应汇率，Proposal 的目标 CPM 与 package 预算统一按 target currency 计算。
...
| 双击旧 `propel.exe` 没反应 | 不要从旧绿色版文件夹单独复制 EXE；改用新的 `propel-1.1.0.exe` 安装包 |
| 8080 端口已经被占用 | `propel` 和 `propel-web` 会自动选择下一个可用本地端口并打开正确地址 |
| Windows 提示未知发布者 | 当前 EXE 未签名；可继续运行，正式外发时建议购买代码签名证书 |
| 需要修改版本号 | 运行 `.\scripts\package-windows.ps1 -AppVersion 1.2.0` |
